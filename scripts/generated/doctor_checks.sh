...
declare -a MANIFEST_CHECKS=(
    ...
    "stack.dolt	Dolt version-control database (required by bd/beads)	dolt version	required	target_user"
    "stack.bd.1	gastownhall beads (bd) - local-first issue tracker	bd --version	required	target_user"
    "stack.bd.2	gastownhall beads (bd) - local-first issue tracker	bd stats 2>/dev/null	optional	target_user"
