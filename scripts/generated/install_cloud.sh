...
wrapper_tmp="$(mktemp "${TMPDIR:-/tmp}/gtbi-wrangler-wrapper.XXXXXX")"
...
gtbi_install_executable_into_primary_bin "$wrapper_tmp" "wrangler"
INSTALL_CLOUD_WRANGLER
        then
...
tmp_dir="$(mktemp -d "${TMPDIR:-/tmp}/gtbi-supabase.XXXXXX")"
tmp_tgz="$(mktemp "${TMPDIR:-/tmp}/gtbi-supabase.tgz.XXXXXX")"
tmp_checksums="$(mktemp "${TMPDIR:-/tmp}/gtbi-supabase.sha.XXXXXX")"
...
gtbi_install_executable_into_primary_bin "$extracted_bin" "supabase"
