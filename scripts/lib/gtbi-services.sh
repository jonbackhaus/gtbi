#!/usr/bin/env bash
# ============================================================
# GTBI Services — Unified background daemon management
# Manages Agent Mail, CM serve, and CASS indexer via tmux
#
# Usage:
#   gtbi services start       Start all services in a tmux session
#   gtbi services stop        Stop the tmux session (kills all)
#   gtbi services status      Show which services are running
#   gtbi services restart     Stop then start
#   gtbi services logs [svc]  Attach to a service pane for logs
#
...
#
# The tmux session is named "gtbi-svc" to avoid conflicts.
...
_info()  { printf '%s[gtbi-services]%s %s\n' "$_C_CYAN" "$_C_RESET" "$*"; }
_ok()    { printf '%s[gtbi-services]%s %s%s%s\n' "$_C_CYAN" "$_C_RESET" "$_C_GREEN" "$*" "$_C_RESET"; }
_warn()  { printf '%s[gtbi-services]%s %s%s%s\n' "$_C_CYAN" "$_C_RESET" "$_C_YELLOW" "$*" "$_C_RESET" >&2; }
_err()   { printf '%s[gtbi-services]%s %s%s%s\n' "$_C_CYAN" "$_C_RESET" "$_C_RED" "$*" "$_C_RESET" >&2; }

_session_exists() {
    tmux has-session -t "$GTBI_SVC_SESSION" 2>/dev/null
}
...
_get_pane_ids() {
    tmux list-panes -t "$GTBI_SVC_SESSION:services" -F '#{pane_id}' 2>/dev/null
}
...
cmd_start() {
    ...
    if _session_exists; then
        _warn "Session '$GTBI_SVC_SESSION' already exists. Services are running."
        _info "Use 'gtbi services status' to check, or 'gtbi services restart' to restart."
        return 0
    fi

    # Pre-flight: check all binaries exist
    local missing=0
    for svc in "${GTBI_SERVICES[@]}"; do
        local cmd_str
    ...
    if $_DRY_RUN; then
        _info "[dry-run] Would create tmux session '$GTBI_SVC_SESSION' with ${#GTBI_SERVICES[@]} panes:"
        for i in "${!GTBI_SERVICES[@]}"; do
            local svc="${GTBI_SERVICES[$i]}"
            _info "  Pane $i ($(_svc_name "$svc")): $(_svc_cmd "$svc")"
        done
        return 0
    fi

    _info "Starting GTBI services in tmux session '$GTBI_SVC_SESSION'..."

    # Create session with a single window named "services"
    tmux new-session -d -s "$GTBI_SVC_SESSION" -n "services"

    # The first pane already exists. Send service 0's command.
    local first_pane_id
    first_pane_id="$(_nth_pane_id 0)"
    tmux send-keys -t "$first_pane_id" "$(_svc_cmd "${GTBI_SERVICES[0]}")" Enter

    # Create additional panes for remaining services
    local i
    for i in $(seq 1 $(( ${#GTBI_SERVICES[@]} - 1 ))); do
        local svc="${GTBI_SERVICES[$i]}"
        ...
        local new_pane_id
        new_pane_id="$(tmux split-window -t "$GTBI_SVC_SESSION:services" -v -P -F '#{pane_id}')"
        tmux send-keys -t "$new_pane_id" "$(_svc_cmd "$svc")" Enter
    done

    # Even out the pane layout
    tmux select-layout -t "$GTBI_SVC_SESSION:services" even-vertical
    ...
    printf '\n'
    _info "Attach with: tmux attach -t $GTBI_SVC_SESSION"
    _info "View logs:   gtbi services logs [agent-mail|cm|cass]"
}
...
cmd_stop() {
    ...
    if ! _session_exists; then
        _info "Session '$GTBI_SVC_SESSION' is not running. Nothing to stop."
        return 0
    fi

    if $_DRY_RUN; then
        _info "[dry-run] Would stop tmux session '$GTBI_SVC_SESSION'"
        return 0
    fi

    _info "Stopping GTBI services..."
    ...
    # Kill the session
    if _session_exists; then
        tmux kill-session -t "$GTBI_SVC_SESSION" 2>/dev/null || true
    fi
...
cmd_status() {
    ...
    if ! _session_exists; then
        printf '%sSESSION%s  %s%s%s  (not running)\n' \
            "$_C_BOLD" "$_C_RESET" "$_C_RED" "$GTBI_SVC_SESSION" "$_C_RESET"
        return 1
    fi

    printf '%sSESSION%s  %s%s%s  (running)\n\n' \
        "$_C_BOLD" "$_C_RESET" "$_C_GREEN" "$GTBI_SVC_SESSION" "$_C_RESET"
    ...
    printf '\n'
    _info "Attach: tmux attach -t $GTBI_SVC_SESSION"
    _info "Logs:   gtbi services logs [agent-mail|cm|cass]"
}

cmd_restart() {
    _info "Restarting GTBI services..."
    cmd_stop
    cmd_start
}
...
cmd_logs() {
    ...
    if ! _session_exists; then
        _err "Session '$GTBI_SVC_SESSION' is not running. Start with: gtbi services start"
        return 1
    fi
    ...
    if [[ -z "$target" ]]; then
        if $_DRY_RUN; then
            _info "[dry-run] Would attach to tmux session '$GTBI_SVC_SESSION'"
            return 0
        fi
        exec tmux attach -t "$GTBI_SVC_SESSION"
    fi
    ...
    if $_DRY_RUN; then
        _info "[dry-run] Would attach to pane $svc_index ($target) in session '$GTBI_SVC_SESSION'"
        return 0
    fi

    exec tmux select-pane -t "$pane_id" \; attach -t "$GTBI_SVC_SESSION"
}
...
Usage: gtbi services <command> [options]

Commands:
  start           Start all GTBI background services
...
Examples:
  gtbi services start              # Start all daemons
  gtbi services status             # Quick health check
  gtbi services logs agent-mail    # View Agent Mail logs
  gtbi services restart            # Restart everything
  gtbi services stop               # Graceful shutdown

The services run in a dedicated tmux session named 'gtbi-svc'.
