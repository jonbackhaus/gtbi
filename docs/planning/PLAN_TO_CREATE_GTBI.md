...
This is the exact flow GTBI must nail:

| Step | User Action | GTBI Responsibility |
|------|-------------|---------------------|
...
| 8 | Paste GTBI one-liner | The magic moment |
...
GTBI consists of **three products in one repo**:
...
**Checkpointing:**
- Progress written to `/var/log/gtbi/install.log`
- State stored in `~/.gtbi/state.json` with completed phases
...
│   └── state.json
├── .zshrc                  # Tiny loader for gtbi.zshrc
...
The manifest (`gtbi.manifest.yaml`) is the **single source of truth** for all tools installed.
...
│       ├── status/page.tsx         # Step 9: GTBI Status Check
...
## Part 8: gtbi doctor Specification
...
`gtbi doctor --json` outputs structured data:
...
- `install.sh` that:
  - Creates `ubuntu` user
  - Installs zsh + shell config via `~/.gtbi`
  - Installs tmux + ntm
...
- `gtbi doctor` shows green across the board
...
Located at `~/.gtbi/zsh/gtbi.zshrc`
...
Type `gtbi doctor` anytime to verify everything is working.
...
The goal is to make agentic engineering accessible to anyone motivated enough to rent a VPS and follow instructions. GTBI is the bridge from "I want to try this AI coding thing" to "I'm running multiple agents coordinating on my project."
